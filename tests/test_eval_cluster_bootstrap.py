...
def test_cluster_bootstrap_ci_is_deterministic_and_brackets_mean():
    ...
    assert lo <= mean <= hi
    assert (lo, mean, hi) == cluster_bootstrap_ci(
        y_true, y_pred, groups, _acc, n_resamples=200, seed=0
    )
...
def test_paired_cluster_bootstrap_detects_positive_gap():
    ...
    lo, mean, _hi = paired_cluster_bootstrap(
        y_true, pred_a, pred_b, groups, _acc, n_resamples=300, seed=0
    )
