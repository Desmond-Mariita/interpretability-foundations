...
# prints the peak-selectivity summary, and runs a self-authored worked example through Pythia.
#
...
# annotates the peak and the earliest depth within the peak's CI (descriptive) for each.
#
...
# - Peak and earliest-within-peak-CI annotations (descriptive; not a hypothesis test)
#
# ### `noun_number` caveat
...
# embedding alone carries orthographic number information. Selectivity increases with
# depth; this describes how the probe's edge over its control grows, not when the
# property first becomes available (it is linearly decodable from the embedding onward).
...
# %% [markdown]
# ## 3. Peak-selectivity summary
#
# The summary shows, for each property, the peak depth point (argmax selectivity over
# the 13-point depth axis) and the earliest depth point whose selectivity 95% CI
# overlaps the peak's CI. The earliest-within-CI point is a **descriptive** rule about
# the selectivity curve: it acknowledges that the argmax is noisy. CI overlap is not a
# hypothesis test, and "earliest" here is not "first availability" -- probe accuracy is
# near ceiling at the embedding layer for all three properties.
#
...
# %%
# ---------------------------------------------------------------------------
# Print peak-selectivity summary
# ---------------------------------------------------------------------------
if metrics_raw:
    props_data = metrics_raw.get("properties", {})
    if props_data:
        print("=== Peak-selectivity summary ===")
        print(f"  {'Property':<14}  {'Peak point':<14}  {'Earliest within peak CI (descriptive)'}")
        print("  " + "-" * 56)
        for _prop in _PROPERTIES:
    ...
    print("(Skipping peak-selectivity summary -- metrics.json not loaded)")
...
# the trained `is_noun` probe to make the depth-profile concept concrete:
#
...
if not _artifacts_ok or not _metrics_ok:
    ...
    if _torch_ok:
        ...
        print(
            ...
            "(the control task baseline) -- but remember:\n"
            "selectivity is necessary-not-sufficient for genuine encoding, and linear\n"
            "availability here is not evidence of causal use."
        )

# %% [markdown]
# ## 5. Discussion
#
# _The full written discussion is in `REPORT.md` section 5 (peak-selectivity summary) and
...
#
# **The earliest-within-peak-CI rule:** we report the earliest depth point whose
# selectivity CI overlaps the peak's CI, not the raw argmax. The argmax is noisy; the
# overlap criterion is a descriptive summary of the selectivity curve -- it is not a
# hypothesis test, and it does not claim the property first becomes linearly available at
# that depth (probe accuracy is near ceiling at the embedding layer).
#
...
# - Last-subword pooling entangles the depth profile with subword count and the plural
#   morpheme.
