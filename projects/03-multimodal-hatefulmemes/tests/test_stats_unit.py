...
def test_repaired_metrics_json_schema_contract():
    """The committed metrics.json carries the corrected schema and provenance."""
    import json

    metrics_path = Path(__file__).resolve().parents[1] / "metrics.json"
    metrics = json.loads(metrics_path.read_text())
    diffs = metrics["auroc_diffs"]
    assert diffs["fused_vs_image"]["point_diff"] == pytest.approx(0.0188, abs=1e-4)
    assert diffs["fused_vs_image"]["ci_low"] is None
    assert diffs["fused_vs_text"]["point_diff"] == pytest.approx(0.1356, abs=1e-4)
    ...
    removed = metrics["provenance"]["removed_prob_array_diffs"]
