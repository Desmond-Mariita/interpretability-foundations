...
# (see `REPORT.md §4.1`); point values and paired CIs are from the 2026-09-22
# reproduction rerun (see `REPORT.md §4` for metric definitions).
...
#
# The committed figure is the two-panel version regenerated by the 2026-09-22 rerun:
# signed share (direction) on the left, magnitude share (dominance) on the right.
...
# Numbers are from `metrics.json["attribution"]`, populated by the 2026-09-22
# reproduction rerun (original per-example values were not cached, which the rerun
# fixed).
...
# - All per-example quantities were regenerated by the 2026-09-22 rerun and are
#   retained under `outputs/` (see `outputs/manifest.json` for hashes and provenance).
